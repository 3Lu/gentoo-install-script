...
loadingModules() {
    ...
    modprobe dm-mod dm-crypt sha256 sha512
}
...
#   echo "Formatting device will be : $[device]"
    ...
    createLVMVolumes $partsize_root $partsize_usr $partsize_var $partsize_opt $partsize_tmp $partsize_home
}

createLVMVolumes() {
    pvcreate /dev/sda3
    vgcreate vg1 /dev/sda3
    lvcreate -L$[1]G -nroot vg1
    lvcreate -L$[2]G -nusr vg1
    lvcreate -L$[3]G -nvar vg1
    lvcreate -L$[4]G -nopt vg1
    lvcreate -L$[5]G -ntmp vg1
    lvcreate -L$[6]G -nhome vg1
}
...
downloadTarBalls() {
    wget -p /mnt/gentoo/ http://de-mirror.org/gentoo/snapshots/portage-latest.tar.bz2
    wget -p /mnt/gentoo/ http://de-mirror.org/gentoo/releases/amd64/autobuilds/current-stage3-amd64-hardened/stage3-amd64-hardened-20140807.tar.bz2
}

extractTarBalls() {
    tar xvjpf /mnt/gentoo/stage3-amd64* -C /mnt/gentoo
